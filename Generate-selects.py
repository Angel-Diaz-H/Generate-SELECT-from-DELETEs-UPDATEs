...
def formatear_y_generar_selects(texto_sql: str):
    # Limpieza de sentencias.
    texto_limpio = re.sub(r"--.*", "", texto_sql) # Quita comentarios.
    texto_unido = " ".join(line.strip() for line in texto_limpio.splitlines() if line.strip()) # Concatena todas las sentencias en una sola línea.
    sentencias = re.split(r"\b(?=delete|update)\b", texto_unido, flags=re.IGNORECASE) # Divide las sentencias en una lista.

    # Agrupa condiciones por tabla y columna. Ej: {tabla: {columna: {valores}}}.
    ...
    for sentencia in sentencias:
        # Valida que las sentencias no esten vacías.
        sentencia = sentencia.strip()
        if not sentencia:
            continue

        # Ignora sentencias no deseadas.
        if sentencia.lower().startswith(("set ", "commit", "rollback")):
            continue

        # Obtiene tablas y campos del Where.
        ...
        if m_update:
            # Extrae solo la tabla.
            tabla = m_update.group(1)
            # Extrae las condiciones where.
        ...
        for cond in condiciones_lista:
            m_cond = re.match(r"([a-zA-Z0-9_]+)\s*(=|>|<|>=|<=|<>|!=|like|in|between)\s*(.+)", cond, re.IGNORECASE)
            if m_cond and m_cond.group(2) == "=":
                col = m_cond.group(1)
    ...
    for tabla, columnas_dict in agrupados.items():
        ...
        for col in orden:
            ...
            # Ordena todos como string para evitar error de comparación.
...
if __name__ == "__main__":
    ...
    if not ruta_entrada.exists():
        ...
        print(f"Archivo generado correctamente en: {ruta_salida.resolve()}")g